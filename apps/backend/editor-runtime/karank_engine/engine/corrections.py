...
def correct_sequence(
    ...
    # مع استثناء البسملة: لا تُسمح إلا مرة واحدة وفي الموضع الأول فقط (element-grammar §1).
    basmala_locked = False
    for i in range(n):
        stripped = lines[i].strip()
        if not stripped:
            continue
        if BASMALA_PATTERN.fullmatch(stripped):
            if i == 0 and not basmala_locked:
                result[i] = State.BASMALA
                basmala_locked = True
            else:
                # بسملة مكررة أو خارج الموضع الأول → تُعامل كسطر وصف عادي.
                result[i] = State.ACTION
            continue
