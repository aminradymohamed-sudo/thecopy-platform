...
def emission_prob(state: State, features: Dict[str, Any]) -> float:
    ...
    if state == State.CHARACTER:
        if features["scene_num"] or features["time_location"] or features["transition"] or features["looks_like_scene_location"]:
            return LOG_ZERO
        # أي مؤشر لفعل سردي يمنع تصنيف السطر كـ CHARACTER حتى لو بدا قصيرًا بنقطتين.
        # هذه حماية ضرورية ضد أخطاء سابقة كانت تسرّب فِعلًا سرديًا (مثل "تدخل سارة")
        # إلى CHARACTER بسبب وجود `:` في نهاية جملة وصفية معطوبة.
