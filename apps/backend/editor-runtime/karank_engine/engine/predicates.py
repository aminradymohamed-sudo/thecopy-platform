...
# نمط لإزالة الإرشاد الأدائي المضمّن قبل فحص اسم الشخصية.
# يطابق أقواس عادية () وعربية كاملة （）.
_INLINE_PARENTHETICAL_STRIP = re.compile(r"\s*(?:\([^\n()]*\)|（[^\n（）]*）)\s*")


def _strip_inline_parenthetical(text: str) -> str:
    """إزالة الإرشاد الأدائي المضمّن من سطر CHARACTER للسماح بفحص الاسم وحده."""
    return _INLINE_PARENTHETICAL_STRIP.sub(" ", text).strip()
...
def looks_like_character_fragment(text: str) -> bool:
    # ندعم صيغة الدمج (element-grammar §6): "مدحت (غاضبًا) :" تبقى CHARACTER واحد.
    # نزيل الإرشاد الأدائي المضمّن قبل فحص الاسم، ثم نطبّق القيود الصارمة على الاسم وحده.
    stripped_raw = strip_trailing_colon(text)
    if not stripped_raw:
        return False
    stripped = _strip_inline_parenthetical(stripped_raw)
    if not stripped:
        return False
    # بعد إزالة الإرشاد الأدائي لا يجب أن تبقى أي أقواس أو رموز محظورة.
