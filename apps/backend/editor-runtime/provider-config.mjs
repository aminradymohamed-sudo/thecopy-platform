...
const getProviderCredentialEnvKeys = (provider, definition) => {
  if (provider === "google-genai") {
    return [...GOOGLE_GENAI_CREDENTIAL_ENV_KEYS];
  }

  return definition?.credentialEnvKey ? [definition.credentialEnvKey] : [];
};
...
export const validateProviderCredential = (provider, env = process.env) => {
  ...
  const credentialEnvKeys = getProviderCredentialEnvKeys(provider, definition);
