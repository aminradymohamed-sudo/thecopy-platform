...
export const getPdfOcrAgentConfig = () => {
  ...
  if (parsed.enableVisionQa) {
    if (!parsed.moonshotApiKey) {
      throw configError(
        "PDF_OCR_CFG_MISSING_MOONSHOT_API_KEY",
        "PDF OCR agent misconfigured: MOONSHOT_API_KEY is required when PDF_OCR_ENABLE_VISION_QA=true."
      );
    }
    ...
    if (!parsed.visionCompareModel) {
      throw configError(
        ...
        "PDF OCR agent misconfigured: PDF_VISION_COMPARE_MODEL is required when PDF_OCR_ENABLE_VISION_QA=true."
      );
    }
    if (!parsed.visionJudgeModel) {
      throw configError(
        "PDF_OCR_CFG_MISSING_VISION_JUDGE_MODEL",
        "PDF OCR agent misconfigured: PDF_VISION_JUDGE_MODEL is required when PDF_OCR_ENABLE_VISION_QA=true."
      );
    }
  }
...
export const getPdfOcrAgentHealth = async () => {
  ...
  const requiresVisionQa = config.enableVisionQa === true;
  ...
  if (requiresVisionQa && !hasMoonshotApiKey)
    errorCodes.push("PDF_OCR_CFG_MISSING_MOONSHOT_API_KEY");
  if (requiresVisionQa && !hasVisionCompareModel)
    errorCodes.push("PDF_OCR_CFG_MISSING_VISION_COMPARE_MODEL");
  if (requiresVisionQa && !hasVisionJudgeModel)
  ...
  return {
    ...
    enableVisionQa: config.enableVisionQa,
