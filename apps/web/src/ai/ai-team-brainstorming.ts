...
export async function aiTeamBrainstorming(
  ...
  return (await aiTeamBrainstormingFlow(input)) as AITeamBrainstormingOutput;
}
...
const aiTeamBrainstormingFlow = ai.defineFlow(
  ...
  async (input: AITeamBrainstormingInput) => {
    const { numberOfAgents, scriptConcept, specificRequests } = input;

    const agentTasks = Array.from(
    ...
    const agentResults = await Promise.all(
      agentTasks.map(async (agentTask) => {
        const taskResult = (await agentTask.task) as {
          output?: { ideas?: string[] };
        };
        return {
